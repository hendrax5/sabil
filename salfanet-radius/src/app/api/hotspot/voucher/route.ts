...
export async function GET(request: Request) {
  ...
  try {
    ...
    // - firstLoginAt/expiresAt: Set by post-auth via new Date() (true UTC).
    //   Send as-is WITHOUT 'Z' so browser renders the raw value without
    //   timezone conversion (avoids double-shift on WIB clients).
...
export async function DELETE(request: Request) {
  try {
    ...
    if (batchCode) {
      ...
      // Delete entire batch (all statuses)
...
// PATCH - Update agent, router, or profile for single or multiple vouchers
export async function PATCH(request: Request) {
  ...
  try {
    ...
    // Build update fields (only update fields that were explicitly supplied)
    const updateData: any = {}
    if (profileId) updateData.profileId = profileId
    if (routerId) updateData.routerId = routerId
    else if (clearRouter) updateData.routerId = null
    if (agentId) updateData.agentId = agentId
    else if (clearAgent) updateData.agentId = null
