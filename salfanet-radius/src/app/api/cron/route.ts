...
import { getCronHistory, recordAgentSales, generateInvoices, sendInvoiceReminders, disconnectExpiredVoucherSessions, reconcileVoucherTransactions } from '@/lib/cron/voucher-sync'
  ...
  try {
    ...
    switch (jobType) {
      ...
      case 'pppoe_session_sync':
        try {
          const { syncPPPoESessions } = await import('@/lib/cron/pppoe-session-sync')
      ...
      case 'sync_online_users':
        try {
          const { syncOnlineUsersFromDB } = await import('@/lib/online-users')
      ...
      case 'suspend_check':
        try {
          const { prisma: suspendPrisma } = await import('@/lib/prisma')
          const now = new Date()
