  ...
  ;(async () => {
    ...
    try {
      ...
      try {
        ...
        await send({ done: true, success: false, l2tp, sstp, pptp, steps, message: `Connection failed: ${e.message}`, rosVersion })
        return
      }
      ...
      sstp = r.ok
      await step(r.ok ? '✅ SSTP Server enabled' : `❌ SSTP: ${r.error}`)
